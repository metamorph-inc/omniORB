...
namespec := _omnicodesets$(PY_MODULE_SUFFIX) _ $(OMNIPY_MAJOR) $(OMNIPY_MINOR)
...
$(lib): $(OBJS)
	...
	$(CXXLINK) $(CXXLINKOPTIONS) -shared -o $@ -Wl,-soname,$(soname) $(IMPORT_LIBRARY_FLAGS) \
	 $(filter-out $(LibSuffixPattern),$^) $(OMNIORB_CODESETS_LIB)\
	)
...
lib = _omnicodesetsmodule.so
libinit = init_omnicodesets
...
$(lib): $(OBJS)
      $(CXXLINK) $(CXXLINKOPTIONS) $(OBJS) $(OMNIORB_CODESETS_LIB) -o $(lib)
...
libname = _omnicodesetsmodule.sl
soname  = $(libname).$(OMNIPY_MAJOR)
lib     = $(soname).$(OMNIPY_MINOR)
...
$(lib): $(OBJS)
	(set -x; \
         ...
         $(filter-out $(LibSuffixPattern),$^) $(OMNIORB_CODESETS_LIB) -lcxxstd -lcxx -lexc -lots -lc \
        )
